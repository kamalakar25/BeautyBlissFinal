...
router.post('/create-booking-notification', async (req, res) => {
  ...
  try {
    ...
    if (!salonShop) return res.status(404).json({ message: 'Service provider not found' });
    ...
    const userNotification = new Notification({
      ...
      message: `Your booking for ${booking.service} on ${new Date(booking.date).toLocaleDateString()} at ${booking.time} is confirmed.`,
      bookingId,
    ...
    const spNotification = new Notification({
      ...
      type: 'Booking',
      title: 'New Booking',
      message: `New booking for ${booking.service} by ${user.name} on ${new Date(booking.date).toLocaleDateString()} at ${booking.time}.`,
      bookingId,
      userDetails: { name: user.name, email: user.email, phone: user.phone },
    });
...
router.post('/mark-notification-read', async (req, res) => {
  ...
  try {
    const notification = await Notification.findOne({ _id: notificationId, recipientEmail: email });
...
router.post('/sp/mark-booking-confirmed', async (req, res) => {
  ...
  try {
    ...
    if (!booking || booking.parlorEmail !== email) {
      return res.status(404).json({ message: 'Booking not found or not associated with this service provider' });
    }
    ...
    await Notification.updateOne(
      ...
      { title: 'Booking Confirmed', message: `You confirmed the booking for ${booking.service}.` }
    );
    ...
    const userNotification = new Notification({
      ...
      message: `Your booking for ${booking.service} on ${new Date(booking.date).toLocaleDateString()} at ${booking.time} has been confirmed by ${salonShop.shopName}.`,
      bookingId,
      userDetails: { name: user.name, email: user.email, phone: user.phone },
    });
